...
public interface MemberMapper {
    ...
    @Mapping(
            target = "status",
            expression = "java(member.getStatus() != null ? member.getStatus().name() : null)"
    )
    @Mapping(
            target = "regdate",
            expression = "java(member.getRegdate() != null ? member.getRegdate().toString() : null)"
    )
    MemberProfileResponseDTO toProfileDto(Member member);
}